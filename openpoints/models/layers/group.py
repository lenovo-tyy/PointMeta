...
class GroupAll(nn.Module):
    ...
    def forward(self, new_xyz: torch.Tensor, xyz: torch.Tensor, features: torch.Tensor = None):
        """
        :param xyz: (B, N, 3) xyz coordinates of the features，特征的xyz坐标
        :param new_xyz: ignored，忽略不用
        :param features: (B, C, N) descriptors of the features，特征的描述符
        :return:
            new_features: (B, C + 3, 1, N)，新的特征
        """
        grouped_xyz = xyz.transpose(1, 2).unsqueeze(2)  # 转置并扩展xyz坐标
        grouped_features = features.unsqueeze(2) if features is not None else None  # 如果特征不为None，则扩展特征
        return grouped_xyz, grouped_features  # 返回分组后的xyz坐标和特征
...
def create_grouper(group_args):
    group_args_copy = copy.deepcopy(group_args)  # 深拷贝分组参数
    method = group_args_copy.pop('NAME', 'ballquery')  # 弹出分组方法，默认为'ballquery'
    radius = group_args_copy.pop('radius', 0.1)  # 弹出半径参数，默认为0.1
    nsample = group_args_copy.pop('nsample', 20)  # 弹出采样点数参数，默认为20

    logging.info(group_args)  # 打印分组参数
    if nsample is not None:  # 如果采样点数不为None
        if method == 'ballquery':  # 如果分组方法为'ballquery'
            grouper = QueryAndGroup(radius, nsample, **group_args_copy)  # 创建QueryAndGroup对象
        elif method == 'knn':  # 如果分组方法为'knn'
            grouper = KNNGroup(nsample,  **group_args_copy)  # 创建KNNGroup对象
        elif method == 'dynamic_knn':  # 如果分组方法为'dynamic_knn'
            grouper = DynamicKNNGroup(nsample,  **group_args_copy)  # 创建DynamicKNNGroup对象
    else:  # 如果采样点数为None
        grouper = GroupAll()  # 创建GroupAll对象
    return grouper  # 返回分组器对象
