...
def three_interpolation(unknown_xyz, known_xyz, know_feat):
    ...
    Args:
    unknown_xyz: torch.Tensor, 未知点云的坐标，大小为(n, 3)
    known_xyz: torch.Tensor, 已知点云的坐标，大小为(m, 3)
    know_feat: torch.Tensor, 已知点云的特征，大小为(m, c)

    Returns:
    interpolated_feats: torch.Tensor, 插值后的特征，大小为(n, c)
    """
    # 找到最近的三个已知点的索引
