...
class BaseSeg(nn.Module):
    def __init__(self,
        ...
        if decoder_args is not None:
            # 将decoder_args与encoder_args合并，更新至decoder_args_merged_with_encoder
            decoder_args_merged_with_encoder = copy.deepcopy(encoder_args)
            decoder_args_merged_with_encoder.update(decoder_args)
            # 将encoder的channel_list属性更新至decoder_args_merged_with_encoder
    ...
    def forward(self, data):
        ...
        if self.decoder is not None:
            if str(self.decoder)[:11] == 'McstDecoder':
