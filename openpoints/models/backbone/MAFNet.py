...
class PointTransformer(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.sa1 = SA_Layer(in_channels)
        self.sa2 = SA_Layer(in_channels)
        self.sa3 = SA_Layer(in_channels)
        self.sa4 = SA_Layer(in_channels)

        self.conv_fuse_1 = nn.Sequential(
                    nn.Conv1d(in_channels*4, 1024, kernel_size=1, bias=False),
                    nn.BatchNorm1d(1024),
                    nn.LeakyReLU(negative_slope=0.2)
                )
        self.conv_fuse_2 = nn.Sequential(
                    nn.Conv1d(3072, out_channels, kernel_size=1, bias=False),
                    nn.BatchNorm1d(out_channels),
                    nn.LeakyReLU(negative_slope=0.2)
                )

        # self.conv = nn.Conv1d(in_channels*6, out_channels, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        x_point_feature = self.conv_fuse_1(x)
        ...
        f = self.conv_fuse_2(torch.cat((x_point_feature, x_global_feature), dim = 1))
...
class Mafusion(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.sa = PointTransformer(in_channels, out_channels)
    ...
    def forward(self, f):
        fa = self.sa(f)
        fm = self.mlp(f)
        f = self.conv_fuse(torch.cat((fa, fm), dim=1))
        return f
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        self.head_conv = (create_conv(channels[0], channels[1],
                                     norm_args=norm_args if not is_head else None,
                                     act_args=None if 0 == len(channels) - 2
                                                      and (self.use_res or is_head) else act_args,
                                     **conv_args))
        convs = []
        for i in range(len(channels) - 1):
            convs.append(create_conv(channels[i], int(channels[i + 1] * 3/4),
                                     norm_args=norm_args if not is_head else None,
                                     act_args=None if i == len(channels) - 2
                                                      and (self.use_res or is_head) else act_args,
                                     **conv_args)
                         )
            self.maf = Mafusion(int(channels[i + 1] * 3/4), channels[i + 1])
