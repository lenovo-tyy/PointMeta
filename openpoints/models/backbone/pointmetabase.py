...
class PointMetaBaseEncoder(nn.Module):
    ...
    def _make_pe_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ...
        参数:
            block (nn.Module): 卷积块的类型。
            channels (int): 输出通道数。
            blocks (int): 该阶段中的块数。
            stride (int): 卷积步长。
            group_args (dict): 分组卷积的参数。
            is_head (bool): 是否是头部阶段。
        返回:
            nn.Sequential 或 nn.ModuleList: 构建的卷积模块。
        """
        # 定义用于位置编码的通道数
        ...
        # 如果该阶段有多个块，则构建卷积层
        if blocks > 1:
            for i in range(len(channels2) - 1):  # 遍历每一对相邻的通道数
                # 创建一个卷积块，并添加到 convs2 列表中
                convs2.append(create_convblock2d(channels2[i], channels2[i + 1],
            ...
            # 如果该阶段只有一个块，则返回一个空的 nn.ModuleList
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        #         with open("points-1.txt", "a+") as f:
        #             f.write(
        #                 "{} {} {} {}\n".format(p0[j, i, 0], p0[j, i, 1], p0[j, i, 2], j))
        #     f.close()
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        p, f = [p0], [f0]
